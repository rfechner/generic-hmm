...
class Preprocessor():
    ...
    def _enforce_timeinterval_consistency(self, dfs : [pd.DataFrame]) -> [pd.DataFrame]:
        ...
        groupby = self._cparser['markerconfig_metainfo']['groupby'] if 'groupby' in self._cparser['markerconfig_metainfo'] else None


        timedelta = abs(pd.Timestamp(time1) - pd.Timestamp(time2))

        for i in range(len(dfs)):
            dfs[i] = self._make_df_timeinterval_consistent(dfs[i], timedelta, dateid, groupby)


        return dfs
        
    def _make_df_timeinterval_consistent(self, df : pd.DataFrame, timedelta : pd.Timedelta, dateid : str, groupby : str) -> pd.DataFrame:
        '''
    ...
    def _build_label_encoders(self):
        ...
        for marker in self._valid_markers:
            ...
            if 'linspace' in dtype:
                ...
                self._df[marker] = self._df[marker].map(lambda x: x.lower().strip().replace(' ', '_'))
            elif dtype == 'date':
                self._df[marker] = self._df[marker].map(lambda x: pd.Timestamp(x))
            else:
                pass
    ...
    def encode_series(self, marker, series : pd.Series):
        ...
        if 'linspace' in dtype:
            ...
            result = series.map(lambda x: x.lower().strip().replace(' ', '_'))
        elif dtype == 'date':
            result = series.map(lambda x: pd.Timestamp(x))
        else:
            result = series
    ...
    def decode_series(self, marker, series : pd.Series):
        ...
        if 'linspace' in dtype or 'date_range' in dtype:    
            result = result.map(lambda x: self._bin_dict[marker][x - 1])
